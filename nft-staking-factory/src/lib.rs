...
const CODE: &[u8] = include_bytes!("../../sputnik-nft-staking/res/sputnik_nft_staking.wasm");
...
pub struct StakingContractFactory {
    contracts: UnorderedSet<AccountId>,
}

impl Default for StakingContractFactory {
    fn default() -> Self {
        env::panic_str("NFT Staking Contract should be initialized before usage")
    }
}

#[near_bindgen]
impl StakingContractFactory {
    #[init]
    pub fn new() -> Self {
        assert!(!env::state_exists(), "The contract is already initialized");
        Self {
            contracts: UnorderedSet::new(b"c".to_vec()),
        }
    }
    ...
    pub fn on_create(
        ...
        if near_sdk::is_promise_success() {
            self.contracts.insert(&account_id);
            true
        } else {
...
mod tests {
    ...
    fn test_basics() {
        ...
        let mut factory = StakingContractFactory::new();
